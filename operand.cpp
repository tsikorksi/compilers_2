...
Operand::Operand(Kind kind)
        ...
        , m_basereg(-1)
        , m_index_reg(-1)
        , m_imm_ival(-1) , callee_register(false){
}
...
Operand Operand::set_callee(int next_vreg) const {
    Operand dup = *this;
    if (next_vreg < 11) {
        dup.m_basereg = next_vreg;
    }
    return dup;
}
Operand Operand::make_callee() const{
    Operand dup = *this;
    dup.callee_register = true;
    return dup;
}
